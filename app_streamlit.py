...
NARRATIVE_KARIR = {
    ...
    "Game Developer": "Game Developer adalah pencipta pengalaman interaktif yang imersif dan menghibur dalam bentuk permainan digital di berbagai platform, mulai dari PC, konsol, hingga mobile. Tugas mereka sangat bervariasi, meliputi implementasi logika permainan (gameplay mechanics), pengaturan sistem fisika (physics engine), integrasi aset visual dan audio, serta optimasi grafis agar game berjalan lancar. Alur kerja mereka biasanya dimulai dari konsep di Game Design Document, kemudian masuk ke tahap prototyping untuk menguji ide utama, dilanjutkan dengan pengembangan penuh menggunakan Game Engine utama seperti Unity (C#) atau Unreal Engine (C++).",
    "DevOps Engineer": "DevOps Engineer adalah jembatan fungsional yang menghubungkan tim pengembangan perangkat lunak (Development) dengan tim operasional IT (Operations). Tujuan utama mereka adalah meningkatkan kecepatan, efisiensi, dan kualitas proses pengiriman perangkat lunak melalui otomatisasi dan kolaborasi. Tugas mereka meliputi otomatisasi CI/CD pipeline, manajemen infrastruktur server, dan pemantauan performa aplikasi di lingkungan produksi menggunakan tools seperti Jenkins, Terraform, Kubernetes, dan Docker di platform cloud seperti AWS, Azure, atau Google Cloud.",
    "Database Administrator": "Database Administrator (DBA) adalah penjaga integritas, keamanan, dan performa penyimpanan data perusahaan. Mereka bertanggung jawab penuh untuk memastikan bahwa basis data tersimpan aman, selalu tersedia saat dibutuhkan, dan dapat diakses dengan cepat serta efisien. Tugas mereka meliputi perancangan skema database, konfigurasi server database, manajemen hak akses pengguna, serta perencanaan backup dan recovery data secara berkala. Mereka bekerja dengan DBMS seperti MySQL, PostgreSQL, Oracle, SQL Server, atau MongoDB."
}
...
@st.cache_resource
def load_model():
    try: return joblib.load('model_karir_it.pkl')
    except: return None
...
def page_home():
    st.markdown("""
    <div style="background:#2c3e50; padding:35px 0; text-align:center; margin-bottom:50px;">
    ...
    with col:
        st.markdown("""<style>
        div[data-testid="stVerticalBlock"] > div:nth-child(1) .stButton > button { background-color: #05c46b !important; font-size:1rem !important; padding: 18px !important; letter-spacing:2px !important; }
        div[data-testid="stVerticalBlock"] > div:nth-child(2) .stButton > button { background-color: #3c40c6 !important; font-size:1rem !important; padding: 18px !important; letter-spacing:2px !important; }
        div[data-testid="stVerticalBlock"] > div:nth-child(3) .stButton > button { background-color: #f39c12 !important; font-size:1rem !important; padding: 18px !important; letter-spacing:2px !important; }
        </style>""", unsafe_allow_html=True)

        if st.button("  ANALISIS KARIR", key="m1", use_container_width=True):
            st.session_state.update({'page':'quiz','q_index':0,'answers':[]}); st.rerun()
        st.markdown("<div style='height:4px'></div>", unsafe_allow_html=True)
        if st.button("  EKSPLORASI BIDANG IT", key="m2", use_container_width=True):
            st.session_state.page = 'encyclopedia'; st.rerun()
        st.markdown("<div style='height:4px'></div>", unsafe_allow_html=True)
        if st.button("  TREN PASAR & GAJI", key="m3", use_container_width=True):
...
def page_quiz():
    # Top bar hijau
    st.markdown('<div style="background:#05c46b;height:8px;margin-bottom:0;width:100%"></div>', unsafe_allow_html=True)
    ...
    if idx < len(LIST_PERTANYAAN):
        ...
        with col:
            ...
            <p style="color:white;font-size:1.2rem;font-weight:bold;text-align:center;
               font-family:Arial,sans-serif;line-height:1.6;margin-bottom:30px;">{teks}</p>
            """, unsafe_allow_html=True)
            st.markdown("""<style>
...
# ==========================================
def page_result():
    if model is None: st.error("Model tidak ditemukan!"); return
    ...
    st.markdown('<div style="background:#3c40c6;height:8px;width:100%"></div>', unsafe_allow_html=True)
    ...
    with cb:
        st.markdown("""<style>.swap-btn .stButton > button {
            background-color:#3c40c6 !important; padding:14px !important;
...
def page_swap():
    ...
    with cm:
        st.markdown("""
        <div style="background:#e74c3c;border-radius:6px;padding:12px 20px;text-align:center;margin-bottom:15px;">
            <span style="color:white;font-weight:700;font-size:0.95rem;">
                ⚠️ Fitur Face Swap hanya tersedia di versi desktop (app_gui.py).<br>
                Download & jalankan di laptop untuk menggunakan fitur ini.
            </span>
        </div>
        """, unsafe_allow_html=True)
        ...
        suffixes_f = [f"{base}_f.jpg", f"{base}_engineer_f.jpg", f"{base}_developer_f.jpg", f"{base}_designer_f.jpg", f"{base}_administrator_f.jpg"]
        suffixes_m = [f"{base}_m.jpg", f"{base}_engineer_m.jpg", f"{base}_developer_m.jpg", f"{base}_designer_m.jpg", f"{base}_administrator_m.jpg"]
        with col_f:
            for s in suffixes_f:
                p = os.path.join(script_dir, "swap_assets", s)
                if os.path.exists(p): st.image(p, caption=f"👩 {result}", use_container_width=True); break
        with col_m:
            for s in suffixes_m:
                p = os.path.join(script_dir, "swap_assets", s)
                if os.path.exists(p): st.image(p, caption=f"👨 {result}", use_container_width=True); break
...
def page_encyclopedia():
    st.markdown("""
    <div style="background:#485460;padding:10px 20px;display:flex;align-items:center;margin-bottom:0;">
    ...
    with col_list:
        for i, nama in enumerate(NARRATIVE_KARIR.keys(), 1):
            ...
            bg = "#3c40c6" if is_active else "#2f3542"
            st.markdown(f"""<style>
            div[data-testid="stVerticalBlock"] div[data-testid="stVerticalBlock"]:nth-child({i}) .stButton > button {{
                background-color: {bg} !important;
                color: {'white' if is_active else '#d2dae2'} !important;
    ...
    with col_detail:
        ...
        st.markdown('<div style="background:#2f3542;padding:15px;border:1px solid #485460;min-height:500px">', unsafe_allow_html=True)
        if os.path.exists(tmpl_path):
            _, ci, _ = st.columns([1, 3, 1])
            with ci:
        ...
        <p style="color:white;font-size:0.95rem;line-height:1.8;font-family:Helvetica,Arial,sans-serif;
           text-align:justify;padding:10px;">{narrative}</p>
        """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
def page_trends():
    st.markdown("""
    <div style="background:#f39c12;padding:10px 20px;margin-bottom:0;">
